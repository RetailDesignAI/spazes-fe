...
import { setSidebarWidth } from '@/providers/redux/ui/uiSlice';
...
const LargeScreenSidebar = () => {
  ...
  const changeSidebarWidth = (width: string) => dispatch(setSidebarWidth(width));
  ...
  const handleOpenClose = () => {
    setIsOpen(!isOpen);
    changeSidebarWidth(isOpen ? SIDEBAR_DEFAULT_WIDTH : SIDEBAR_MIN_WIDTH);
  };
    ...
    <>
      ...
      >
        ...
        <div className="w-full py-3 logo">
          <h1 className="h-[10%] text-xl font-bold text-center text-white">{isOpen ? 'Spazes' : 'S'}</h1>
        </div>
